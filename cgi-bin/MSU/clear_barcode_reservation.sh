#!/bin/bash
clear_barcode_reservation () {
    echo "Rack Info"
    echo "<br>"
    echo '<pre>'
    sudo /opt/butler_server/erts-9.3.3.6/bin/escript /home/gor/rpc_call.escript reservationinfo delete "[{$1,$2}]."
    echo '</pre>'
}
...
echo "<br>"
  ...
  if [ -z "$QUERY_STRING" ]; then
   ...
   # No looping this time, just extract the data you are looking for with sed:
     ...
     echo '<br>'
     clear_barcode_reservation $XX $YY
  fi
echo '</body>'
echo '</html>'
